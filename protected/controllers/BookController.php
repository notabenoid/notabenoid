...
//				$this->redirect($this->book->getUrl("members"));
    ...
    private function members_requests()
    {
        ...
        if (count($rDecline) + count($rAccept) > 0) {
            $sql = "DELETE FROM group_queue WHERE book_id = '{$this->book->id}' AND user_id IN('".implode("', '", array_merge($rDecline, $rAccept))."')";
            Yii::app()->db->createCommand($sql)->execute();
        }
    ...
    private function members_invite($user_list)
    {
        ...
        foreach ($users as $u) {
            if ($u->membership->status == GroupMember::MEMBER ||
                ...
                $u->membership->status == GroupMember::BANNED) {
                continue;
            }
...
//				if($status == GroupMember::MODERATOR and $member->status == GroupMember::CONTRIBUTOR) continue;
        ...
        if (count($delete) > 0) {
            Yii::app()->db->createCommand("DELETE FROM groups WHERE book_id = '{$this->book->id}' AND user_id IN(".implode(',', $delete).')')->execute();
        }
        if (count($update) > 0) {
            foreach ($update as $k => $V) {
                Yii::app()->db->createCommand("UPDATE groups SET status = '{$k}' WHERE book_id = '{$this->book->id}' AND user_id IN(".implode(',', $V).')')->execute();
            }
        }
    ...
    public function actionDict_rm($book_id)
    {
        ...
        if ($dict->book_id != $book->id) {
            throw new CHttpException(403, 'Вы пытаетесь удалить слово из другого перевода. Нехорошо-с.');
        }
