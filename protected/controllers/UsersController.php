...
class UsersController extends Controller
{
    ...
    public function init()
    {
        ...
        if (p()['registerType'] == 'INVITE' && !$user->isGuest && $user->id == $_GET['id']) {
            ...
            if ($user->model->n_invites > 0) {
                $this->submenu['invites'] .= ' ('.$user->model->n_invites.')';
            }
        }
        if ($user->can('admin')) {
    ...
    private function loadUser($id)
    {
        ...
        if ($user->isDeleted) {
            throw new CHttpException(404, 'Пользователь удалён.');
        }
    ...
    public function actionComments($id)
    {
        ...
        elseif ($mode == 'tblog') {
            $c->addCondition('t.orig_id IS NULL     AND post.book_id IS NOT NULL');
        } elseif ($mode == 'tr') {
            $c->addCondition('t.orig_id IS NOT NULL');
        }
...
//				"condition" => "t.book_id IS NULL OR (book.ac_read = 'a' AND book.ac_blog_r = 'a')",
    ...
    public function actionInviteCode($id, $iid)
    {
        ...
        if (!$invite) {
            throw new CHttpException(404, 'Инвайт не найден');
        }
    ...
    public function actionDelete($id)
    {
        ...
        if ($id != $user->id) {
            $this->redirect($user->getUrl('delete'));
        }
        if ($_POST['really']) {
