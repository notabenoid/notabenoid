...
class BookBlogController extends BookBaseController
{
    ...
    public function actionIndex($book_id)
    {
        ...
        if (isset($_GET['topic'])) {
            ...
            if (!isset(Yii::app()->params['blog_topics']['book'][$topic])) {
    ...
    public function actionPost($book_id, $post_id)
    {
        ...
        if (!$post) {
            throw new CHttpException(404, "Поста не существует. Возможно, он удалён. Почитайте лучше <a href='".$this->book->getUrl('blog')."'>другие посты</a> в блоге этого перевода.");
        }
        $post->book = $this->book;
    ...
    public function actionComment_reply($book_id, $post_id, $comment_id = 0)
    {
        ...
        if (!$this->book->can('blog_c')) {
            throw new CHttpException(403, 'Вы не можете оставлять комментарии в блоге этого перевода. '.$this->book->getWhoCanDoIt('blog_c'));
        }
