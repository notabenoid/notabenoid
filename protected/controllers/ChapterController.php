...
class OrigCountFixer
{
    ...
    {
        if (count($this->sql) >= $this->max_updates) {
            return false;
        }
...
class ChapterController extends Controller
{
    ...
    public function actionGo($book_id, $chap_id, $nach, $ord)
    {
        ...
        if ($nach == 'prev') {
            $sql = 'SELECT id FROM chapters WHERE book_id = :book_id AND ord < :ord ORDER BY ord desc LIMIT 1';
        } else {
    ...
    public function actionTimeshift($book_id, $chap_id)
    {
        ...
        if (!$chap->book->can('chap_edit')) {
            throw new CHttpException(403, 'Вы не можете изменять оригинал в этом переводе.');
        }
        if ($chap->book->typ != 'S') {
...
//			), $chap, $options);
        ...
        if ($rcKey === false || $data === false || $modified >= $rcMTime) {
            ...
            if ($options->enc != 'UTF-8') {
                $data = iconv('UTF-8', $options->enc.'//TRANSLIT', $data);
            } else {
                $data = sprintf('%c%c%c', 0xEF, 0xBB, 0xBF).$data;
            }
    ...
    public function actionImport_subs($book_id, $chap_id)
    {
        ...
        if (!isset($_POST['ImportOptionsSubs'])) {
            $this->redirect($chap->getUrl('import'));
        }
...
//				exit;
    ...
    {
        $chap = $this->loadChapter($book_id, $chap_id);
        if (!$chap->can('rate') || !$chap->can('trread')) {
            throw new CHttpException(403, 'Вы не можете оценивать версии в этом переводе.');
        }
...
//			throw new CHttpException(403, "Вы не можете ставить оценки переводам потому что у вас отрицательная карма.");
        ...
        if ($tr->user_id == $user->id) {
            throw new CHttpException(403, 'Нельзя ставить оценки собственным переводам.');
        }
