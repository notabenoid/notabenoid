...
//		}
    ...
    public function actionChpid($id)
    {
        ...
        if (!$parent) {
            throw new CHttpException(404, 'Родительский раздел удалён.');
        }
        ...
        if ($parent->id == $handle->id) {
            ...
            if ($debug) {
                echo "Handle: {$handle->dump}<br />";
            }
            if ($debug) {
                echo "Parent: {$parent->dump}<br />";
            }
            ...
            if ($debug) {
                echo 'mp_base = ['.implode(',', $mp_base)."] ('{$t}')<br />";
            }
