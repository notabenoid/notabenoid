...
class OrigController extends Controller
{
    ...
    protected function loadOrig($book_id, $chap_id, $orig_id, $with = null)
    {
        ...
        // ac_read для всего перевода. Если нельзя в весь перевод, редиректим в оглавление перевода, пусть контроллер Book объясняет, почему да как.
        if (!$orig->chap->book->can('read')) {
            $this->redirect($orig->chap->book->url);
        }
    ...
    {
        if ($orig_id == 0) {
            $this->redirect('/book/'.intval($book_id).'/'.intval($chap_id));
        }
    ...
    public function actionComment_edit($book_id, $chap_id, $orig_id, $comment_id)
    {
        ...
        if (!$comment) {
            throw new CHttpException(404, 'Комментарий, вероятно, удалён.');
        }
        if (!$comment->can('edit')) {
            throw new CHttpException(403, 'Свои комментарии можно редактировать только в течение часа после их написания. Вероятно, время вышло.');
        }
    ...
    public function actionComment_rate($book_id, $chap_id, $orig_id, $comment_id)
    {
        $orig = $this->loadOrig($book_id, $chap_id, $orig_id);
        if (!$orig->chap->can('comment')) {
            throw new CHttpException(403, 'Вы не можете оценивать комментарии в этом переводе.');
        }
