...
class MaintainCommand extends CConsoleCommand
{
    ...
    {
        if (count($this->profileInfo) <= 0) {
            echo "profileStop called without profileStart\n";
        }
...
*/
        ...
        foreach ($origs as $o) {
            ...
            if ($n_fixed % 1000 == 0) {
                printf("Fixed records: %d (%d%%), INS: %d, UPD: %d\r", $n_fixed, ($n_fixed / $n) * 100, $n_inserted, $n_updated);
            }
            $n_fixed++;
        }
