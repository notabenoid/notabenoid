...
TTT;
    ...
    if ($this->ini['l.metascheme'] == 1) {
        $metaColors = ['#888', '#888'];
    } elseif ($this->ini['l.metascheme'] == 2) {
        $metaColors = ['#888', '#000'];
    } elseif ($this->ini['l.metascheme'] == 3) {
        $metaColors = ['#000', '#000'];
    } else {
        $metaColors = ['#bbb', '#888'];
    }
...
TTT;
        ...
        if ($this->ini['c.sc'] != 0) {
            $css .= $this->getCssComments($this->ini['c.sc']);
        }
    ...
    public function getCssComments($scheme_id, $containerClass = '.comments')
    {
        ...
        if (isset($scheme['backGround'])) {
            $css .= "{$containerClass} .comment.new {background-color:{$scheme['backGround']}}\n";
        }
        if (isset($scheme['metaColor'])) {
            $css .= "{$containerClass} .comment.new .meta, {$containerClass} .comment.new .meta a {color:{$scheme['metaColor']}}\n{$containerClass} .comment.new a.ajax { border-bottom-color:{$scheme['metaColor']}}\n";
        }
        if (isset($scheme['textColor'])) {
            $css .= "{$containerClass} .comment.new {color:{$scheme['textColor']}}\n";
        }
