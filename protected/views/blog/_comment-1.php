<?php
     ...
     * @var bool    - не выводить рейтинг
     * @var bool    - не выводить кнопки для голосования
    ...
    if ($comment->isDeleted()) {
        ...
        if (!$disable_reply and !Yii::app()->user->isGuest) {
            echo "<a href='#cmt_{$comment->id}' class='re ajax'>Ответить</a> ";
        }

        if (!$disable_dot) {
            echo "<a href='#' class='dot b'><i class='i icon-flag'></i></a> ";
        }
        if (!$disable_delete and $comment->can('delete')) {
            echo "<a href='#' class='rm b'><i class='i icon-remove'></i></a>  ";
        }

        if (!$disable_rating) {
            echo "<div class='rating'>";
            if (!$disable_rater && $comment->can('rate')) {
                echo "<a class='p' href='#'>+</a>";
            }
            echo '<span>'.str_replace('-', '&minus;', $comment->rating).'</span>';
            if (!$disable_rater && $comment->can('rate')) {
                echo "<a class='n' href='#'>&minus;</a>";
            }
            echo '</div>';
        }
    ...
    if (!$disable_up && $comment->pid && !$comment->isDeleted()) {
        echo "<a href='#cmt_{$comment->pid}' class='up'><i class='i icon-up'></i></a> ";
    }
