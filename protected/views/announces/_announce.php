    ...
    <div class='b'>
        ...
        <p class='info'>
            <?php
                ...
                if ($announce->book->cat_id) {
                    echo " в разделе &laquo;<a href='/search?cat={$announce->book->cat_id}'>{$announce->book->cat->title}</a>&raquo;";
                }
                ...
                if ($announce->n_comments > 0) {
                    ...
                    if (!Yii::app()->user->isGuest) {
                        echo "<a href='{$announce->url}#Comments' title='Написать комментарий'><i class='icon-nb-comment'></i></a> ";
                    }
                }
                echo '</span> ';
            ?>
