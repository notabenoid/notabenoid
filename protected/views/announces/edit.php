...
echo $form->textAreaRow($post, 'body', ['class' => 'span6', 'hint' => 'Здесь можно использовать HTML-теги a, b, strong, i, em, u, small, sub, sup.']);
?>
<div class="form-actions">
<?php
    echo CHtml::htmlButton("<i class='icon-ok icon-white'></i> Сохранить", ['type' => 'submit', 'class' => 'btn btn-primary']).' ';
    if (!$post->isNewRecord && Yii::app()->user->can('blog_moderate')) {
        echo CHtml::htmlButton("<i class='icon-ban-circle icon-white'></i> Удалить", ['class' => 'btn btn-danger remove']).' ';
    }
    if ($post->isNewRecord) {
