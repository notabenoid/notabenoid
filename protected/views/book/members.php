<?php
    ...
    if ($book->can('membership')) {
        Yii::app()->getClientScript()->registerScriptFile('/js/GM.js');
    }
?>
...
<?php
    if ($book->membership->status == 0) {
        ...
        if (count($A) > 0) {
            foreach ($A as $ac) {
                if ($group_needed == '') {
                    $group_needed = 'В этом переводе нужно состоять в группе переводчиков, чтобы ';
                } else {
                    $group_needed .= ', ';
                }
...
<?php
    ...
    foreach ($members as $member) {
        ...
        echo '<td>';
        if ($member->membership->status == GroupMember::MODERATOR) {
            echo "<i class='icon-briefcase' title='Модератор'></i> ";
        }
        echo $member->ahref;
        if ($member->id == $book->owner_id) {
            echo ' (создатель)';
        } elseif ($member->membership->status == GroupMember::CONTRIBUTOR and $book->facecontrol != Book::FC_OPEN) {
            echo ' (не в группе)';
        } elseif ($member->membership->status == GroupMember::BANNED) {
            echo ' (забанен)';
        }
        echo '</td>';
        ...
        echo '<td>';
        if ($book->can('trread')) {
            echo "<a href='".$member->getUrl("translations/{$book->id}")."'>";
        }
        echo $member->membership->n_trs;
        if ($book->can('trread')) {
            echo '</a>';
        }
        if ($book->n_vars > 0 and $member->membership->n_trs > 0) {
            printf(' <small>(%d%%)</small>', $member->membership->n_trs / $book->n_vars * 100);
        }
        echo '</td>';

        echo '<td>';
        if ($member->membership->rating < 0) {
        ...
        if ($member->membership->n_trs != 0) {
            printf('%.2f', $member->membership->rating / $member->membership->n_trs);
        }
        echo '</td>';
