...
<div id='Info'>
	<?php
        ...
        if ($book->cat_id || $book->can('book_edit')) {
            echo "<p class='cat'>";
            if ($book->cat_id) {
                echo "<a href='{$book->cat->url}'>{$book->cat->pathHtml}</a>";
            }
            if ($book->can('book_edit') || Yii::app()->user->can('cat_moderate')) {
                echo "<span> &larr; <a href='".$book->getUrl('edit/cat')."' class='act'>указать раздел каталога</a></span>";
            }
            echo '</p>';
        }

        if ($book->descr != '') {
            echo '<p>'.Yii::app()->parser->out($book->descr).'</p>';
        }
    ?>
...
<?php
    ...
    foreach ($chapters as $chap) {
        ...
        if ($book->typ == 'S') {
            echo '<td>';
            if ($chap->n_verses > 0) {
                echo "<a href='".$chap->getUrl('orig')."' title='Скачать оригинал'>&laquo;&laquo;&laquo;</a>";
            }
            echo '</td>';
        }
...
//			if($chap->status) echo " &mdash; " . Yii::app()->params["translation_statuses"][$chap->status];
        ...
        echo '<td>';
        if ($chap->status) {
            echo Yii::app()->params['translation_statuses_short'][$chap->status];
        }
        echo '</td>';
        ...
        if ($chap->n_verses == 0) {
            echo "<td colspan='3'>";
            if ($book->can('chap_edit')) {
                echo "<i class='icon-upload'></i> <a href='".$chap->getUrl('import')."'>импортировать оригинал</a>";
            } else {
                echo '(пусто)';
            }
            echo '</td>';
            ...
            if ($chap->d_vars > 0) {
                $title = sprintf(" title='КП=%.01f'", $chap->n_vars / $chap->d_vars);
            } else {
                $title = '';
            }
            ...
            if ($chap->n_vars > 0) {
                ...
                if ($chap->n_dl > 0) {
                    ...
                    if ($chap->n_dl_today > 0) {
                        $tdl .= " (сегодня &ndash; {$chap->n_dl_today})";
                    }
                }
                if ($chap->can('gen')) {
