    ...
    echo "<div style='text-align:right; margin-top:5px;'><a href='".$book->getUrl('edit/access')."' class='act'>Редактировать</a></div>";
        ...
        <dd><?php
            if ($book->n_vars == 0 || $book->n_verses == 0) {
                ...
                if ($book->d_vars > 0 and $book->n_vars > 0) {
                    printf(" <abbr title='Коэффициент Плюрализма: среднее количество вариантов перевода каждого фрагмента'>КП</abbr> = %.01f", $book->n_vars / $book->d_vars);
                }
                echo '</div>';
        ...
        <?php
            if (!Yii::app()->user->isGuest) {
                ...
                if ($myStatus != '') {
                    echo "<dt>Ваш статус:</dt><dd>{$myStatus}</dd>";
                }
            }
        ?>
    </dl>
...
<?php
    ...
    if (!Yii::app()->user->isGuest) {
        $Tools[] = "<a href='".$book->getUrl('recalc')."'>Пересчитать статистику перевода</a>";
    }
?>
<div class='tools'>
