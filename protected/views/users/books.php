...
<?php
    foreach ($groups as $group) {
        ...
        echo '<td>';
        if ($group->status == GroupMember::MODERATOR) {
        ...
        if ($group->book->owner_id == $user->id) {
            echo ' (создатель)';
        }
        echo '</td>';
        ...
        echo '<td>';
        if ($group->n_trs != 0 && $group->book->ac_read == 'a' && $group->book->ac_trread == 'a') {
            echo "<a href='".$user->getUrl("translations/{$group->book->id}")."'>";
        }
        echo $group->n_trs;
