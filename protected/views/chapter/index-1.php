                ...
                <?php
                    foreach ($filter::$modes as $k => $v) {
                        ...
                        if ($k == 2) {
                            ...
                            if (!$user->isGuest) {
                                echo "title='Ctrl+I: вставить ваш ник' ";
                            }
                            echo '/>';
                        } elseif ($k == 5) {
    ...
    <?php
        ...
        if ($orig_dp->totalItemCount > $p->pageSize) {
            ...
            for ($i = 1; $i <= $p->pageCount; $i++) {
                ...
                if ($cur) {
                    echo "<input type='text' name='Orig_page' value='{$i}' accesskey='g' />";
                } else {
                    echo "<a href='?".http_build_query($g)."'>{$i}</a>";
                }
                echo '</li>';
            }
            echo '</ul> ';
            ...
            if ($p->currentPage + 1 >= $p->pageCount) {
                echo '<span>&rarr;</span>';
            } else {
...
<?php
    ...
    if ($orig_dp->totalItemCount == 0):
        ...
        if ($filter->show == 0 || $chap->n_verses == 0) {
            ...
            if ($chap->book->can('chap_edit')) {
                echo " Если хотите, вы можете <a href='".$chap->getUrl('import')."'>загрузить его</a> или <a href='#' class='create'>создать первый фрагмент</a>.";
            }
            echo '</p>';
        } else {
    ...
    if (!$user->isGuest && $chap->status == Chapter::STATUS_READY) {
        ...
        if ($chap->book->can('owner')) {
            echo ' Впрочем, вы &mdash; владелец перевода и можете делать тут, что захотите.';
        } elseif ($chap->book->can('chap_edit')) {
            echo " Вы, так как сами являетесь модератором, можете изменить статус перевода, нажав на кнопку <i class='icon-pencil'></i> в <a href='{$chap->book->url}'>оглавлении перевода</a>.";
        }
        echo '</p>';
    }

    if (Yii::app()->user->ini['t.hlr'] != 0) {
        $tableClasses[] = 'has-best';
    }
        ...
        if ($tableEmpty) {
            ...
            foreach ($orig as $o) {
                ...
                if ($o->n_comments > 0) {
                    ...
                    if ($n_new) {
                        echo "<i class='icon-comment-new'></i> ";
                    } else {
                        echo "<i class='icon-comment'></i>";
                    }
                    if ($o->seen->n_comments) {
                        echo "{$o->seen->n_comments}<br />";
                    }
                    if ($n_new) {
                        echo "<b class='n'>+{$n_new}</b>";
                    }
                } else {
                    if ($chap->can('comment')) {
                        echo "<i class='i icon-comment-empty'></i>";
                    }
                }
                echo '</a>';
                echo '</td>';
