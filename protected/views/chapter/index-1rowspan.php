    ...
    <div class="btn-group pull-right" style='vertical-align: top'>
        <div id='progress-info'>
            ...
            if ($chap->n_verses == 0) {
                ...
                $procent = floor($chap->d_vars / $chap->n_verses * 1000) / 10;
            }
...
if ($orig_dp->totalItemCount == 0):
    if ($show == 0 || $chap->n_verses == 0) {
        ...
        if ($chap->book->can('chap_edit')) {
            echo " Не желаете ли <a href='".$chap->getUrl('import')."'>сделать это сейчас</a>?";
        }
        echo '</p>';
    } else {
...
<table id="Tr" class="translator translator-orig-editing translator-tr-editing">
	...
	<?php
        ...
        function renderTrRow($tr, $chap)
        {
            ...
            echo "<td class='tr'>";
            if ($chap->can('rate')) {
                echo "<div class='rater'>";
            }
            if ($chap->can('rate') && $chap->book->membership->status == GroupMember::MODERATOR) {
        ...
        foreach ($orig as $o) {
            ...
            if ($chap->book->typ == 'S') {
                if ($show == 0) {
                    echo "<a href='#' class='ord'>#{$o->ord}</a> &middot; ";
                }
            ...
            if ($show != 0) {
                echo " <a href='{$o->url}' class='ctx'>в контексте</a>";
            }
            echo '</div>';
            ...
            if ($o->n_comments > 0) {
                ...
                if ($chap->can('comment')) {
                    echo "<a href='#' class='c add' title='Написать комментарий'><i class='icon-nb-comment'></i></a> ";
                }
            }
            echo '</td>';
