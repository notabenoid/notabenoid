...
//		echo "</ul>";
    ...
    <p><?php
        if ($chap->n_vars == 0 || $chap->n_verses == 0) {
            echo '&mdash;';
        } else {
            ...
            if ($chap->d_vars > 0 and $chap->n_vars > 0) {
                printf(" <abbr title='Коэффициент Плюрализма: среднее количество вариантов перевода каждого фрагмента'>КП</abbr> = %.01f", $chap->n_vars / $chap->d_vars);
            }
            echo '</div>';
