...
class User extends CActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            // on => "register"
            ['login, pass, lang', 'required', 'message' => 'Укажите {attribute}.', 'on' => 'register'],
    ...
    {
        if (is_array($this->ini)) {
            $this->ini = strrev(implode('', $this->ini));
        }
        if (is_array($this->can)) {
            $this->can = strrev(implode('', $this->can));
        }
    }

    protected function afterFind()
    {
        if (!is_array($this->upic)) {
            $this->upic = sscanf($this->upic, '{%d,%d,%d}');
        }
        $this->bits_unpack();
