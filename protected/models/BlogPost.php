...
class BlogPost extends CActiveRecord
{
    ...
    public function common($topics = null)
    {
        ...
        if (count($topics) == 0) {
            ...
            $this->dbCriteria->addInCondition('topics', $topics);
        }
    ...
    public function can($what)
    {
        ...
        if ($what == 'edit') {
            if ($this->book_id != 0) {
                if ($this->isAnnounce) {
                    return Yii::app()->user->can('blog_moderate');
                } else {
                    return $this->user_id == $user->id || $this->book->membership->status == GroupMember::MODERATOR;
                }
            } else {
                return $this->user_id == $user->id || $user->can('blog_moderate');
            }
        } elseif ($what == 'read') {
            if (!isset(Yii::app()->params['blog_topics']['common'][$this->topics])) {
                return false;
            }
