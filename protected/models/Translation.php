...
class Translation extends CActiveRecord
{
    ...
    public function afterSave()
    {
        ...
        if (count($chap_update) > 0) {
            $sql[] = 'UPDATE chapters SET '.implode(', ', $chap_update).' WHERE id = :chap_id;';
            $params[':chap_id'] = $this->chap_id;
        }

        if (count($book_update) > 0) {
            $sql[] = 'UPDATE books SET '.implode(', ', $book_update).' WHERE id = :book_id;';
    ...
    public function afterDelete()
    {
        ...
        $this->chap->n_vars--;
        $this->chap->book->n_vars--;
        if ($was_last) {
            $this->chap->d_vars--;
            $this->chap->book->d_vars--;
        }
    ...
    public function render($opts, $filter = null)
    {
        ...
        if ($opts['rate']) {
            $html .= "<div class='rater'>";
        }
        if ($opts['rate'] && $opts['rate-']) {
            $html .= "<a href='#' class='m'>&minus;</a> ";
        }
        $html .= "<span class='rate'>{$this->rating}</span>";
        if ($opts['rate']) {
            $html .= " <a href='#' class='p'>+</a></div>";
        }
        ...
        if ($this->user_id == 0) {
            ...
            $html .= " ({$this->user->ahref})";
        }

        if ($opts['edit']) {
            $html .= " <a href='#' class='e'><i class='icon-edit'></i></a>";
        }
        if ($opts['rm']) {
            $html .= " <a href='#' class='x'><i class='icon-remove'></i></a>";
        }
        $html .= "</div>\n";
