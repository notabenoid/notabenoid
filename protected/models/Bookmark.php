...
//	}
    ...
    public function relations()
    {
        return [
            ...
            'book' => [self::BELONGS_TO, 'Book',     'book_id'/* , "select" => array("id", "owner_id", "s_title", "t_title", "n_vars", "d_vars", "n_verses") */],
            'orig' => [self::BELONGS_TO, 'Orig',     'orig_id'],

            'no' => [self::STAT, 'Bookmark', 'book_id', 'condition' => 't.user_id = no.user_id AND no.orig_id IS NOT NULL'],
        ];
    }
    ...
    public function afterDelete()
    {
        if (!$this->orig_id) {
            Yii::app()->db->createCommand('DELETE FROM bookmarks WHERE user_id = :user_id AND book_id = :book_id AND orig_id IS NOT NULL')
                ->execute([':user_id' => $this->user_id, ':book_id' => $this->book_id]);
        }
    ...
    public static function set($book_id, $orig_id = null, $note = null)
    {
        ...
        $bm = self::model()->findByAttributes($pk);
        ...
        if (!$bm) {
            ...
            $bm->ord = Yii::app()->db->createCommand('SELECT MAX(ord) FROM bookmarks WHERE user_id = :user_id AND orig_id IS NULL')->queryScalar([':user_id' => Yii::app()->user->id]) + 1;
        }
