...
class Chapter extends CActiveRecord
{
    ...
    public function can($what)
    {
        ...
        if ($what == 'gen_untr') {
            ...
            if ($this->d_vars / $this->n_verses >= 0.95) {
                return true;
            }
            if ($this->book->ac_read != 'a' || ($this->ac_read != '' && $this->ac_read != 'a')) {
                return true;
            }
        ...
        $ac = 'ac_'.$what;
        if (!$this->$ac) {
            return $this->book->can($what);
        }

        if ($this->$ac == 'a') {
            // "a" разрешает анонимам только read и gen
            if (Yii::app()->user->isGuest and !($what == 'read' || $what == 'gen')) {
                return false;
            } else {
                return true;
            }
        ...
        if ($this->$ac == 'g') {
            return $this->book->membership->status == GroupMember::MEMBER or $this->book->membership->status == GroupMember::MODERATOR;
        }
        if ($this->$ac == 'm') {
            return $this->book->membership->status == GroupMember::MODERATOR;
        }
        if ($this->$ac == 'o') {
            return $this->book->owner_id == $user->id;
        }
    ...
    public function idle_time_text()
    {
        ...
        if ($t < 60) {
            return $this->idle_time.' сек.';
        }
        if ($t < 60 * 60) {
            return round($this->idle_time / 60).' мин.';
        }
        if ($t < 60 * 60 * 24) {
            return round($this->idle_time / 3600).' час.';
        }
        if ($t < 60 * 60 * 24 * 30) {
            return round($this->idle_time / 86400).' дней.';
        }
        if ($t < 60 * 60 * 24 * 30 * 12) {
            return round($this->idle_time / 2592000).' мес.';
        }
    ...
    public function getWhoCanDoIt($what = 'read', $tools = true)
    {
        ...
        if ($this->$ac == '') {
            return $this->book->getWhoCanDoIt($what, $tools);
        }
    ...
    public function getDeniedWhy()
    {
        ...
        if ($this->ac_read == 'o') {
            $msg = 'Владелец перевода ('.$this->book->owner->ahref.') закрыл доступ в эту главу для всех.';
        } elseif ($this->ac_read == 'm') {
            $msg = 'Эта глава доступна только модераторам этого перевода, которых назначает владелец ('.$this->book->owner->ahref.').';
            ...
            $msg = 'Вы не можете войти в эту главу перевода.';
        }
