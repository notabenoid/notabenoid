var Profile = {
    uid: 0,
    book_leave: function(book_id) {
        if (!confirm("Вы уверены, что хотите покинуть эту группу? Возможно, для того, чтобы вернуться, вам понадобится снова дожидаться приглашения от её создателя или подтверждения вашей кандидатуры модераторами.")) return false;
        console.log("leave group id=%d", book_id);
        $.post("/book/" + book_id + "/members_leave", {
            ajax: 1
        }, function(data) {
            location.href = "/users/" + Profile.uid
        });
        return false;
    }
};
