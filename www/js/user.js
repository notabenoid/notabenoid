var BM = {
    ...
    init: function() {
        ...
        $("#bookmarks").delegate("#bookmarks-list li .rm", "click", function(e) {
            ...
            $li.find("button.cancel").click(function() {
                $li.html(save_html);
            });
    ...
    loadIni: function() {
        ...
        if (!ini) return;
        var a = ini.split(".");
        BM.order = parseInt(a[0]);
        BM.title_part = a[1];
        BM.moder_only = parseInt(a[2]);
    },
    saveIni: function() {
        $.cookie("bm-ini", [BM.order, BM.title_part, BM.moder_only].join("."), {
            expires: 365,
            path: "/"
        });
    },
    ...
    sort: function() {
        var sort_f = [
            ...
            /* 2 */
            function(a, b) {
                a = parseInt(a.group.last_tr);
                if (isNaN(a)) a = 0;
                b = parseInt(b.group.last_tr);
                if (isNaN(b)) b = 0;
                return b - a;
            },
            /* 3 */
            function(a, b) {
                a = parseInt(a.group.since);
                if (isNaN(a)) a = 0;
                b = parseInt(b.group.since);
                if (isNaN(b)) b = 0;
                return b - a;
            },
            /* 4 */
            function(a, b) {
                a = parseFloat(a.book.ready);
                if (isNaN(a)) a = 0;
                b = parseFloat(b.book.ready);
                if (isNaN(b)) b = 0;
                return b - a;
            },
            /* 5 */
            function(a, b) {
                a = parseFloat(a.cdate);
                if (isNaN(a)) a = 0;
                b = parseFloat(b.cdate);
                if (isNaN(b)) b = 0;
                return b - a;
            }
        ];
